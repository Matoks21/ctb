...
public class FSM {
    ...
    private String resultStringForming(String bank, boolean usd, boolean eur, int rounding,
        ...
        String bankString = switch (bank) {
            case "NBU" -> convert("NBU");
            case "Monobank" -> convert("Monobank");
            case "PB" -> convert("PB");
            default -> throw new Exception("Invalid bank name '" + bank + "', must be one of NBU, PB, Mono.");
        };
        if (!usd && !eur) {
            result.append(convert("Currency notification is not selected")).append("\n\n");
        } else {
            result.append(convert("Currency exchange rate in "))
                    .append(bankString)
                    .append(":\n\n");
        }
        if (usd) {
